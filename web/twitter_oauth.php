...
if(!empty($_GET['oauth_verifier']) && !empty($_SESSION['oauth_token']) && !empty($_SESSION['oauth_token_secret'])){
	...
	if(isset($user_info->error) || $user_info->id == 0 || !is_numeric($user_info->id)){
        ...
        if (!($stmt = $mysqli->prepare("SELECT id,userid,oauth_token,oauth_secret FROM tokens WHERE userid = ?"))) {
        ...
        $result = array();
        $stmt->bind_result($result['id'],$result['userid'],$result['oauth_token'],$result['oauth_secret']);
        $stmt->fetch();
		...
		if(empty($result)){
            ...
            $result = $mysqli->query("SELECT id,userid,oauth_token,oauth_secret FROM tokens WHERE id = " . $mysqli->insert_id);
